...
def loop():
    global estado, tiempo, start_time, interval, input_seq, desactivacion_seq
    while True:
        if estado == STATE_CONFIG:
            ...
            if shake_sensor.is_gesture("shake"):
                ...
                input_seq = []  # Reiniciar secuencia al armar la bomba
        elif estado == STATE_ARMED:
            if btn_up.was_pressed():
                input_seq.append("A")
            if btn_down.was_pressed():
                input_seq.append("B")
            if shake_sensor.was_gesture("shake"):
                #input_seq.append("SHAKE")
                pass
